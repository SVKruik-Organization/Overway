...
export default defineEventHandler(async (event): Promise<{
    ...
    try {
        ...
        const response: Array<Array<{
            "object_id": any; // BigInt
            "object_type": UserTypes;
        }>> = await connection.query(`
            SELECT object_id, object_type FROM sessions WHERE payload = ?;
        ...
        return {
            "object_id": Number(response[0][0].object_id),
            "object_type": response[0][0].object_type,
        };
    } catch (error: any) {
